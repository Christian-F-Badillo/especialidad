...
reg.conf.intervals <- function(x, y) {
    ...
    plot(x, y, cex = 1.75, pch = 21, bg = 'gray', xlab = "Salary", ylab = "YearsExperience")
...
conf.intervals <- reg.conf.intervals(data$YearsExperience, data$Salary)

CT <- (data$Salary - mean(data$Salary))^2

SCT <- sum(SCT)

fit <- lm(Salary~YearsExperience, data = data)
