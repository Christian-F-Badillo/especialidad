...
knitr::kable(conteo_difedad, caption = "Diferencia de edad entre contrayentes.", col.names = c(
    "Diferencia en Edad", "Total"), align = "cc")
```

```{r}
#| echo: false
#| fig-cap: "Diferencia de edad entre contrayentes."

ggplot(conteo_difedad, aes(x =diferencia_edad, y = cantidad))+
  geom_bar(stat = "identity", fill = "forestgreen", color = "black")+
  theme_minimal()+
  labs( 
       x = "Diferencia de edad (años)", 
       y= "Total") +
  scale_x_continuous(breaks = seq(0, max(conteo_difedad$diferencia_edad), 5))
```

### Diferencia de edad en Matrimonios por estado.
...
knitr::kable(head(conteo_difedad, 10), caption="Diferencia de edad en contrayentes por estado (primeras obs.).",
             col.names = c("No. Estado", "Estado", "Diferencia de Edad", "Total"), align = "clcc")
```
...
knitr::kable(head(conteo_difedad, 10), 
             caption="Rango de diferencia en edad de contrayentes por estado (primeras obs.).",
             col.names = c("No. Estado", "Estado", "Rango en Diferencia de Edad", "Total"), 
             align = "clcc")
```
...
#| echo: false
#| label: fig-charts
#| fig-cap: "Diferencia de edad entre contrayentes por estado."
#| fig-subcap:
#|   - "Diferencia de edad entre contrayentes por estado (0-9 años)."
#|   - "Diferencia de edad entre contrayentes por estado (10-19 años)."
#|   - "Diferencia de edad entre contrayentes por estado (20-29 años)."
#|   - "Diferencia de edad entre contrayentes por estado (30-39 años)."
#|   - "Diferencia de edad entre contrayentes por estado (>40 años)."
#| layout: [[47, -6, 47], [47, -6, 47], [50]]
#| layout-align: center
...
crear_grafico(rangos[1])
crear_grafico(rangos[2])
crear_grafico(rangos[3])
crear_grafico(rangos[4])
crear_grafico(rangos[5])
```
...
knitr::kable(head(escolaridad, 10), 
...
```{r}
#| echo: false
#| fig-cap: "Último nivel de estudios de los contrayentes por estado."
#| fig-width: 14
#| fig-height: 14

ggplot(escolaridad, aes(x= factor(estado, levels = unique(estado)), y = cantidad))+
  geom_bar(stat ="identity", fill = "forestgreen", col ="black")+
  facet_wrap(~ escolaridad_abrv, nrow = 4, ncol = 4, scales = "free_y") + 
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust= 1))+
  labs(
       x = "Estado",
...
## Matrimonios por entidad


## Matrimonios por mes

Se estiman los matrimonios por mes para la muestra.
...
knitr::kable(resultados.total.mat, caption="Estimación del total de matrimonios por Mes",
             col.names = c("Mes", "Valor Real", "Estimación Puntual", "IC Inferior", "IC Superior"), 
             align = "ccccc")
```
...
```{r}
#| echo: false
#| fig-cap: "Estimación del total de matrimonios por Mes."
#| fig-width: 8
#| fig-height: 5
...
Podemos observar que a excepción del mes de diciembre, todos los intervalos de confianza contienen el valor real del total de matrimonios por mes, sin embargo, existe un alto grado de incertidumbre en las estimaciones, lo cual puede ser resultado del método de muestreo o que por azar no se obtuvieron suficientes datos para cada mes para mejorar la precisión de las estimaciones.

## Matrimonios por mes por entidad federativa


## Matrimonios con contrayentes de sexos opuestos


## Matrimonios con contrayentes del mismo sexo (mujeres)

Estimamos el total de matrimonios con contrayentes mujer-mujer.
...
knitr::kable(resultados.total.MM.mat, caption="Estimación del total de matrimonios con contrayentes mujer-mujer.",
             col.names = c("Valor Real", "Estimación Puntual", "IC Inferior", "IC Superior"), 
             align = "cccc")
```

Existe una alta incertidumbre en la estimación debido a la poca cantidad de observaciones en la muestra usada (`r MM.mat[2]` observaciones).

```{r}
#| echo: false
#| fig-cap: "Estimación del total de matrimonios con contrayentes mujer-mujer."
#| fig-width: 8
#| fig-height: 5
...
## Matrimonios con contrayentes del mismo sexo (hombres)


## Proporción de matrimonios con contrayentes del mismo sexo por entidad federativa


## Matrimonios entre mexicanos

Estimamos el total de matrimonios con contrayentes que tienen ambos la nacionalidad mexicana.
...
knitr::kable(resultados.total.Nac.Mat.Mex, 
             caption="Estimación del total de matrimonios entre contrayentes mexicanos.",
             col.names = c("Valor Real", "Estimación Puntual", "IC Inferior", "IC Superior"), 
             align = "cccc")
```

Dado que una gran cantidad de matrimonios en la muestra son con contrayentes de nacionalidad mexicana (`r Nac.Mat.Mex[2]` observaciones), la incertidumbre de nuestra estimación es muy baja.

```{r}
#| echo: false
#| fig-cap: "Estimación del total de matrimonios con ambos contrayentes mexicanos."
#| fig-width: 8
#| fig-height: 5
...
##  Matrimonios de mexicanos con extranjeros


##  Promedio de edad contrayentes mujeres

Estimamos la edad promedio de las contrayentes mujeres.
...
knitr::kable(resultados.mean.con.MM, 
             caption="Estimación de la edad promedio de las contrayentes mujeres.",
             col.names = c("Valor Real", "Estimación Puntual", "IC Inferior", "IC Superior"), 
             align = "cccc")
```

Dado que tenemos una gran cantidad de información de la edad de contrayentes mujeres (`r length(Edad)` observaciones) podemos tener un estimado bastante preciso de su edad promedio.

```{r}
#| echo: false
#| fig-cap: "Estimación de la edad promedio de las contrayentes mujeres."
#| fig-width: 8
#| fig-height: 5
...
##  Promedio de edad contrayentes hombre


##  Promedio de edad contrayentes hombres por entidad federativa


##  Promedio de edad contrayentes mujeres por entidad federativa
...
knitr::kable(edad.p.entidad.MM,
             caption="Estimación de la edad promedio de las contrayentes mujeres por estado.",
             col.names = c("Entidad", "Estimación Puntual", "Error", "IC Inferior", "IC Superior", "Valor Real"),
             align = "cccccc")
```

```{r}
#| echo: false
#| fig-cap: "Estimación de la edad promedio de las contrayentes mujeres por estado."
#| fig-width: 8
#| fig-height: 5
...
Los intervalos de confianza al 95% de todos los estados a excepción de Chihuahua (Chih) contienen a los valores reales de la población. Existe heterogeneidad entre el rango de los intervalos de confianza para los estados, dado que nuestro muestreo fue aleatorio simple y no garanztiza una muestra de igual tamaño para los distintos estados del pais, por ello, se concluye que esta diferencia se debe a la cantidad de información obtenida de cada estado en la muestra. El caso más extremo de esto se ve en la estimación de Nayarit (Nay), Baja California Sur (BCS), Tlaxcala (Tlax), donde el intervalo es bastante amplio. Otra posible explicación de la incertidumbre en las estimaciones es la densidad poblacional, dado que la cantidad de matrimonios es proporcional al total de la población de cada estado.
...
knitr::kable(res.act.lab.con,
             caption="Estimación de la actividad laboral de los contrayentes.",
             col.names = c("Valor Real", "Codificación de Act. Lab.", "Estimación Puntual", 
                           "IC Inferior", "IC Superior", "Actividad Laboral", "Abreviación"),
             align = "ccccclc")
```
\normalsize
...
```{r}
#| echo: false
#| fig-cap: "Estimación de la actividad laboral de los contrayentes."
#| fig-width: 8
#| fig-height: 5
...
A excepción de la estimación en "Insuficientemente Especificado" (InsEsp) y "Busca Trabajo" (BuscaT), las estimaciones son muy buenas y su intervalo de confianza contiene el valor real de la población.
...
Estimamos la escolaridad de los contrayentes estratificado por entidad federativa.
...
knitr::kable(head(Esc.Total.by.Ent, 15),
             caption="Estimación de la escolaridad de los contrayentes por estado.",
             col.names = c("No. Entidad", "Codificación de Escolaridad", "Estimación Puntual",
                           "IC Inferior", "IC Superior", "Escolaridad","Abreviación", "Estado", "Valor Real"),
             align = "lcccclccc")
```
\normalsize
...
```{r}
#| echo: false
#| fig-cap: "Estimación de la escolaridad de los contrayentes por estado."
#| fig-width: 15
#| fig-height: 14
...
ggplot(Esc.Total.by.Ent, aes(x= factor(Estado, levels = unique(Estado)), y = Real.Val, colour = "blue"))+
    ...
    facet_wrap(~ Escol.Name, nrow = 9, ncol = 2, scales = "free") + 
    labs(x = "Estado",
         y = "Total",
...
Existen problemas visibles en las estimaciones para los rangos que corresponden al nivel de primaria y sin escolaridad, lo cual es explicado a la falta de datos de estos niveles de la variable "Escolaridad" de nuestra muestra. Dado que tenemos 288 estratos (32 estados y 9 niveles de educación), en algunos de ellos no se cuentan con datos o son insuficientes para realizar estimaciones precisas, lo que se ve reflejado en muchos valores reales de la población que no están dentro del intervalo de confianza. 